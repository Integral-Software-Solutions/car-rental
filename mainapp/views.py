...
def economical_cars(request):
    cars = CarInfo.objects.filter(
        carname__icontains='wagonr',
        availability='available'
    ) | CarInfo.objects.filter(
        carname__icontains='swift',
        availability='available'
    ) | CarInfo.objects.filter(
        carname__icontains='alto',
        availability='available'
    )
    return render(request, "car_categories/economical_cars.html", locals())

def suv_cars(request):
    cars = CarInfo.objects.filter(
        carname__icontains='creta',
        availability='available'
    ) | CarInfo.objects.filter(
        carname__icontains='scorpio',
        availability='available'
    ) | CarInfo.objects.filter(
        carname__icontains='bolero',
        availability='available'
    )
    return render(request, "car_categories/suv_cars.html", locals())

def luxury_cars(request):
    cars = CarInfo.objects.filter(
        carname__icontains='bmw',
        availability='available'
    ) | CarInfo.objects.filter(
        carname__icontains='audi',
        availability='available'
    ) | CarInfo.objects.filter(
        carname__icontains='mercedes',
        availability='available'
    )
    return render(request, "car_categories/luxury_cars.html", locals())

def convertible_cars(request):
    cars = CarInfo.objects.filter(
        carname__icontains='convertible',
        availability='available'
    ) | CarInfo.objects.filter(
        carname__icontains='roadster',
        availability='available'
    )
    return render(request, "car_categories/convertible_cars.html", locals())
